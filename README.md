...
Run `npm install` once, then `npm run check` for lint, type, and format checks (ESLint, `tsc` with `checkJs` over `src/`, and `prettier --check`). Only `src/` is pushed by `clasp`.

Run `npm test` for unit tests (`tests/*.test.js`, via Node's built-in `node --test`) covering the library's pure logic: sheet include/exclude resolution, timestamped file name formatting, the IMPORTRANGE-formula detection regex, IMPORTRANGE spill-region detection, and PDF query-string building. Apps Script runtime calls (`SpreadsheetApp`, `DriveApp`, `UrlFetchApp`, `Utilities.formatDate`) aren't covered by these — test those manually from the Apps Script editor: call `exportSpreadsheetToPdfFile({ spreadsheetId, excludeSheets: [...] }, folderId)` (and a run using `beforeExport`) against a scratch spreadsheet, then open the result. Confirm the sheet passed via `excludeSheets` is absent from the PDF, columns passed via `hideColumns` (index, letter, and `'E:G'` range forms) are absent while formulas depending on them still show correct values, any `beforeExport` change (e.g. a collapsed row group) is reflected, the exported file name's date/time suffix is correct, and the original spreadsheet is completely unchanged afterward.
